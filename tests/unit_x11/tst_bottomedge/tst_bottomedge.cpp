...
private Q_SLOTS:
    ...
    void test_commit_when_onethird_passed()
    {
        ...
        QPoint delta(0, -(bottomEdge->height() / 3 + UCUnits::instance().gu2(6)));
    ...
    void test_collapse_when_dragged_downwards_data()
    {
        ...
        // upwards
        for (int i = 0; i < 10; i++) {
            shortPath << QPointF(0, -UCUnits::instance().gu2(3)).toPoint();
            longPath << QPointF(0, -UCUnits::instance().gu2(7)).toPoint();
        }
        // downwards
        for (int i = 0; i < 5; i++) {
            shortPath << QPointF(0, UCUnits::instance().gu2(2)).toPoint();
            longPath << QPointF(0, UCUnits::instance().gu2(2)).toPoint();
        }
    ...
    void test_drag_ends_in_uncovered_region_collapses()
    {
        ...
        QPoint from(bottomEdge->width() / 2.0f, bottomEdge->height() - 1);
        QPoint to = from + QPoint(0, -(bottomEdge->parentItem()->height() - UCUnits::instance().gu2(10)));
