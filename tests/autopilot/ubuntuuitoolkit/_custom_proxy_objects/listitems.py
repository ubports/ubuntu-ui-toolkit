...
class ListItem(_common.UbuntuUIToolkitCustomProxyObjectBase):
    """Base class to emulate swipe for leading and trailing actions."""

    def setUp(self):
        super(ListItem, self).setUp()
        print("HELLO")

    def _drag_content(self, direction):
        x, y, width, height = self.globalRect
        start_x = x + (width * 0.2)
        if direction == 'left':
            stop_x = x - (width * 0.8)
    ...
    def _swipe_into_direction(self, direction):
        if direction == 'left':
            raise _common.ToolkitException(
                'Only swiping to the right will cause the item to be deleted.')
        elif direction != 'right':
            raise _common.ToolkitException(
                'Invalid direction "{0}" used on swipe to delete function'
                .format(direction))
        self._drag_content(direction)

    @autopilot_logging.log_action(logger.info)
    def swipe_leading(self):
        """Swipe the item in from left to right to open leading actions."""
        if self.leadingAction:
            self._swipe_into_direction("right")
        else:
            raise _common.ToolkitException(
                'No leadingAction on "{0}"'.format(self.objectName))
