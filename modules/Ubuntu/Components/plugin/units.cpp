    ...
    QObject(parent)
{
    ...
    m_densityBuckets[ldpi.name] = ldpi;
    m_densityBuckets[mdpi.name] = mdpi;
    m_densityBuckets[hdpi.name] = hdpi;
    m_densityBuckets[xhdpi.name] = xhdpi;
...
{
    return m_bucket.scaleFactor;
}

QString Units::bucket()
{
    return m_bucket.name;
}
...
{
    m_bucket = m_densityBuckets[bucketName];
    Q_EMIT scaleFactorChanged();
    Q_EMIT bucketChanged();
}

QString Units::selectBucket(float density, QString formFactor)
{
    ...
    Q_FOREACH (Bucket bucket, m_densityBuckets) {
