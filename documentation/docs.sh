...
grep -v "error: Output file already exists; overwriting" $DOC_PATH/qdoc.log | grep -v "qdoc: warning: No documentation for 'global'" | grep -v "Maybe you meant" | grep -v "but there was no topic command" | grep -v " warning: " | grep -v "warning: Can't link to" > $DOC_PATH/qdoc.err
