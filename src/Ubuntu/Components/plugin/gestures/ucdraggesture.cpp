...
namespace {
const char *statusToString(UCDragGesture::Status status)
{
    if (status == UCDragGesture::WaitingForTouch) {
        return "WaitingForTouch";
    } else if (status == UCDragGesture::Undecided) {
        return "Undecided";
    } else {
        return "Recognized";
    }
...
public:
    ...
    {
        return type == UCDragGesture::Upwards || type == UCDragGesture::Downwards;
    }
    ...
    , m_activeTouches(m_timeSource)
{
    ...
    connect(this, &QQuickItem::enabledChanged, this, &UCDragGesture::giveUpIfDisabledOrInvisible);
    connect(this, &QQuickItem::visibleChanged, this, &UCDragGesture::giveUpIfDisabledOrInvisible);
}
...
void UCDragGesture::setMaxDeviation(qreal value)
{
...
qreal UCDragGesture::wideningAngle() const
{
    return m_wideningAngle;
}

void UCDragGesture::setWideningAngle(qreal angle)
{
...
void UCDragGesture::setDistanceThreshold(qreal value)
{
...
void UCDragGesture::setMinSpeed(qreal value)
{
    if (m_minSpeed != value) {
        m_minSpeed = value;
...
void UCDragGesture::setMaxSilenceTime(int value)
{
...
void UCDragGesture::setCompositionTime(int value)
{
...
void UCDragGesture::setRecognitionTimer(UbuntuGestures::AbstractTimer *timer)
{
    int interval = 0;
    ...
    connect(timer, &UbuntuGestures::AbstractTimer::timeout,
            this, &UCDragGesture::checkSpeed);
    if (timerWasRunning) {
        m_recognitionTimer->start();
    }
}

void UCDragGesture::setTimeSource(const SharedTimeSource &timeSource)
{
...
void UCDragGesture::updateSceneDistance()
{
...
void UCDragGesture::touchOwnershipEvent(TouchOwnershipEvent *event)
{
    if (event->gained()) {
        QVector<int> ids;
...
void UCDragGesture::unownedTouchEvent(UnownedTouchEvent *unownedTouchEvent)
{
...
void UCDragGesture::unownedTouchEvent_undecided(UnownedTouchEvent *unownedTouchEvent)
{
    const QTouchEvent::TouchPoint *touchPoint = fetchTargetTouchPoint(unownedTouchEvent->touchEvent());
    if (!touchPoint) {
        qCritical() << "UCDragGesture[status=Undecided]: touch " << m_touchId
...
void UCDragGesture::touchEvent_absent(QTouchEvent *event)
{
...
void UCDragGesture::touchEvent_undecided(QTouchEvent *event)
{
...
void UCDragGesture::touchEvent_recognized(QTouchEvent *event)
{
    const QTouchEvent::TouchPoint *touchPoint = fetchTargetTouchPoint(event);

    if (!touchPoint) {
        qCritical() << "UCDragGesture[status=Recognized]: touch " << m_touchId
...
void UCDragGesture::watchPressedTouchPoints(const QList<QTouchEvent::TouchPoint> &touchPoints)
{
...
bool UCDragGesture::recognitionIsDisabled() const
{
    return distanceThreshold() <= 0 && compositionTime() <= 0;
}

void UCDragGesture::emitSignalIfTapped()
{
...
const QTouchEvent::TouchPoint *UCDragGesture::fetchTargetTouchPoint(QTouchEvent *event)
{
    const QList<QTouchEvent::TouchPoint> &touchPoints = event->touchPoints();
...
bool UCDragGesture::pointInsideAllowedArea() const
{
...
bool UCDragGesture::movingInRightDirection() const
{
...
bool UCDragGesture::movedFarEnough(const QPointF &point) const
{
...
void UCDragGesture::giveUpIfDisabledOrInvisible()
{
    if (!isEnabled() || !isVisible()) {
...
void UCDragGesture::setStatus(UCDragGesture::Status newStatus)
{
    if (newStatus == m_status)
        return;

    UCDragGesture::Status oldStatus = m_status;
...
void UCDragGesture::setPreviousPos(const QPointF &point)
{
...
void UCDragGesture::setPreviousScenePos(const QPointF &point)
{
...
void UCDragGesture::updateVelocityCalculator(const QPointF &scenePos)
{
...
bool UCDragGesture::isWithinTouchCompositionWindow()
{
    return
        compositionTime() > 0 &&
...
UCDragGesture::ActiveTouchesInfo::ActiveTouchesInfo(const SharedTimeSource &timeSource)
    : m_timeSource(timeSource)
{
}

void UCDragGesture::ActiveTouchesInfo::update(QTouchEvent *event)
{
...
#if ACTIVETOUCHESINFO_DEBUG
QString UCDragGesture::ActiveTouchesInfo::toString()
{
    QString string = "(";
...
void UCDragGesture::ActiveTouchesInfo::addTouchPoint(int touchId)
{
...
qint64 UCDragGesture::ActiveTouchesInfo::touchStartTime(int touchId)
{
    qint64 result = -1;
...
void UCDragGesture::ActiveTouchesInfo::removeTouchPoint(int touchId)
{
    m_touchInfoPool.forEach([&](Pool<ActiveTouchInfo>::Iterator &touchInfo) {
        if (touchId == touchInfo->id) {
...
qint64 UCDragGesture::ActiveTouchesInfo::mostRecentStartTime()
{
...
void UCDragGesture::updateSceneDirectionVector()
{
...
qreal UCDragGesture::projectOntoDirectionVector(const QPointF &sceneVector) const
{
