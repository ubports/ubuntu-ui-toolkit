    ...
    , m_flags(Stretched)
{
    ...
    QObject::connect(&UCUnits::instance(this), SIGNAL(gridUnitChanged()), this,
...
{
    const float gridUnitInDevicePixels = UCUnits::instance(this).gridUnit() / qGuiApp->devicePixelRatio();
...
QSGNode* UCUbuntuShape::updatePaintNode(QSGNode* oldNode, UpdatePaintNodeData* data)
{
    ...
    if (m_relativeRadius == 0) {
        ...
        radius = UCUnits::instance(this).gridUnit() * radiusGuMap[m_radius]
